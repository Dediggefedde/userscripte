...
function postf(){
	...
	console.log(charnamelist,aktquest);
	
	for(var i=0;i<charnamelist.length;i++)if(charnamelist[i][0]==aktquest)aktcharnames.push(charnamelist[i][1]);
	...
	document.getElementById("postform").addEventListener("submit",function(){ 
		...
		GM_setValue("charnamelist",charnamelist.join(String.fromCharCode(12)));
	},false);
	
	if(aktcharnames.length==0)return;
...
function showzipper(){
	...
	var element=""+
		...
		"<div class='tgchan_zip_author'><label for='tgchan_zip_author_text'>Author (divide by ,)</label><input type='text' value='"+firstauthor+"' id='tgchan_zip_author_text'/></div>"+
...
function html2texalt(text, isauthor){
	...
	for(var i=0;i<text.length;i++){
		if(companfalt(akttext,i,"$$>$$")){
			...
			i+="$$<$$/span".length-1;
			commandon=true;
		}else if(akttext.substring(i).search(/^\s*?..<..img[^>]*?src="(.*?)"/i)!=-1){
			var bildpfad=akttext.substring(i).match(/^\s*?..<..img[^>]*?src="(.*?)"/i)[1];
			ausgabe+="\\includegraphics[]{"+bildpfad.substring(bildpfad.lastIndexOf("/")+1)+"}";
			i+="$$<$$img".length-1;
			commandon=true;
			if(downpic&&downloadinlinebild.indexOf(bildpfad)==-1)rest++
			if(downpic&&downloadinlinebild.indexOf(bildpfad)==-1)getimagedata("http://tgchan.org"+bildpfad,bildpfad.substring(bildpfad.lastIndexOf("/")+1));
			downloadinlinebild.push(bildpfad);
	...
	return ausgabe.replace(/^\s|\s$|\s*\\\\\s*$/g,"");
}
...
if(laden("charnamelist")!=null){charnamelist=laden("charnamelist").split(String.fromCharCode(12));
	for(var i=0;i<charnamelist.length;i++)charnamelist[i]=charnamelist[i].split(String.fromCharCode(11));
}
